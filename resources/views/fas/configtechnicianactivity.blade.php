        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    <li class="breadcrumb-item active" aria-current="page">Technician Activity List</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-12 col-lg-4 d-flex">
                    <div class="card border shadow-none w-100">
                        <div class="card-body">
                            <form class="row g-3" method="POST" action="{{ url('/FAS/add-technician-activity') }}">
                                @csrf
                                <div class="col-12">
                                    <label class="form-label">Description</label>
                                    <input type="text" class="form-control @error('tech_activity') is-invalid @enderror" name="tech_activity" value="{{ old('tech_activity') }}" autofocus>
                                    @error('tech_activity')
                                ...
                                <table class="table align-middle" id="mytbl">
                                    ...
                                    <tbody class="text-wrap">
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $data->tech_activity }}</td>
                                            <td>
                                                <div class="d-flex align-items-center gap-3 fs-6">
                                                    <a href="javascript:;" id="edit" data-editid="{{ $data->id }}" data-editview="{{ $data->tech_activity }}" class="text-warning" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="Edit info" aria-label="Edit"><i class="bi bi-pencil-fill"></i></a>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"> Update Technician Activity</h5>
...
<script>
    $(document).ready(function() {
        ...
        $('#editsave').click(function(e) {
            e.preventDefault();
            var url = "{{ url('FAS/technician-activity') }}" + "/" + $('#eid').val();
        ...
        $(document).on("click", "#delete", function(e) {
            var url = "{{ url('/FAS/deleteta') }}" + "/" + $(e.currentTarget).data("deleteid");
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                }
